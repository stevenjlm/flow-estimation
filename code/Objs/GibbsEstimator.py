#!/usr/bin/env python
"""
The GibbsEstimator class computes likely parameters from the measurements
"""
# ================================================== Standard Packages
    ...
    def _ogWeightsGuess(self):
        ...
        for iWeight in range(0, self.iSzWeights):
            for jWeight in range(0, self.jSzWeights):
                self.We[iWeight, jWeight] = self._drawWeightSample(
                    iWeight, jWeight)
            ...
            for tStep in range(1, Measurement.nTimeSteps - 1):
                posterior_t[tStep, iParamStep] = posterior_t[
                    tStep - 1, iParamStep] * norm.pdf(
                        ...
                        testVerse.M[Measurement.node, tStep + 1],
                        Measurement.vv)
        ...
        for iWeight in range(0, self.iSzWeights):
            for jWeight in range(0, self.jSzWeights):
                if Wl[iWeight, jWeight] == Wu[iWeight, jWeight]:
                    ...
                    self.thetas[(iWeight,
                                 jWeight)] = np.array([Wl[iWeight, jWeight]])
                else:
                    wu = Wu[iWeight, jWeight]
                    wl = Wl[iWeight, jWeight]
                    stepSize = (wu - wl) / float(nSteps - 1)

                    self.thetas[(iWeight, jWeight)] = np.arange(
                        wl, wu + 0.5 * stepSize, stepSize)
