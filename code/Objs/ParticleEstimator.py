...
class ParticleEstimator:
    ...
    def _ogWeightsGuess(self, iParticle):
        ...
        for iWeight in range(0, self.iSzWeights):
            for jWeight in range(0, self.jSzWeights):
                self.We[iWeight, jWeight,
                        iParticle] = self._drawOgWeightSample(
                            iWeight, jWeight)
    ...
    def _resample(self, tStep):
        graphWeightsBar = np.zeros((self.iSzWeights, self.jSzWeights,
                                    self.nParticles))
        for iParticle in range(0, self.nParticles):
            particleIdx = self._drawParticleNumber()
            graphWeightsBar[:, :, iParticle] = np.squeeze(
                self.We[:, :, particleIdx])
    ...
    def plotGraphWeightsDistribution(self, i, j, tStep):
        ...
        plt.savefig(
            filename, transparent=True)  # save as pdf for higher quality
        plt.close()
    ...
    def estimateParameters(self, Universe, Measurement, Wl, Wu):
        ...
        ==============================
        """
        self.We[:, :, 0] = np.array([[1, 0, 1], [0, 0, 1], [0, -1.45, 0]])
        # ============================
        ...
        self.log.info(
            'Done particle filtering! Here it goes.. The average weight particle is,'
        )
