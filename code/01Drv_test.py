...
if __name__ == '__main__':
    ...
    for j in range(0, nsteps):
        ...
        for tStep in range(1, 7):
            posterior_t[tStep, j] = posterior_t[tStep - 1, j] * norm.pdf(
                M2hat[tStep + 1], Mtest[2, tStep + 1], vv)

    for tStep in range(0, 8):
        if (np.sum(posterior_t[tStep, :]) / float(nsteps)) != 0:
            posterior_t[tStep, :] = posterior_t[tStep, :] / (
                np.sum(posterior_t[tStep, :]) / float(nsteps))
        else:
        ...
        tempText = 'The most probable parameter value at time step ' + str(
            tStep) + ' is:'
        log.info(tempText)
        log.info(paramMax)
